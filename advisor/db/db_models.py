...
class Base(DeclarativeBase):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        for column in self.__table__.columns:
            ...
            result[column.name] = value.value if isinstance(value, SQLEnum) else value  # type: ignore
        return result
...
class User(Base):
    ...
    external_id: Mapped[str] = mapped_column(PG_UUID(as_uuid=True), default=uuid4, unique=True, nullable=False)
    ...
    normalized_transactions: Mapped[list["NormalizedTransaction"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    categories: Mapped[list["Category"]] = relationship(cascade="all, delete-orphan")
    budget_thresholds: Mapped[list["BudgetThreshold"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    financial_period_snapshots: Mapped[list["FinancialPeriodSnapshot"]] = relationship(back_populates="user", cascade="all, delete-orphan")
...
class RawTransaction(Base):
    ...
    normalized_transaction: Mapped["NormalizedTransaction" | None] = relationship(back_populates="raw_transaction")

    __table_args__ = (
        Index("ix_raw_transaction_user_date", "user_id", "date"),
    )
...
class NormalizedTransaction(Base):
    ...
    recurrence_status: Mapped[RecurrenceStatus] = mapped_column(SQLEnum(RecurrenceStatus), nullable=False)
    recurrence_confidence: Mapped[Decimal | None] = mapped_column(Numeric(precision=5, scale=4), nullable=True)
    recurrence_period: Mapped[PeriodEnum] = mapped_column(SQLEnum(PeriodEnum), nullable=False)
    ...
    financial_period_snapshot_id: Mapped[int | None] = mapped_column(ForeignKey("FINANCIAL_PERIOD_SNAPSHOT.id"), nullable=True, index=True)
    financial_period_snapshot: Mapped["FinancialPeriodSnapshot" | None] = relationship(back_populates="transactions")
    ...
    __table_args__ = (
        Index("ix_normalized_transaction_user_date", "user_id", "date"),
    )
...
class BudgetThreshold(Base):
    ...
    # Source and status
    source: Mapped[BudgedThresholdSourceEnum] = mapped_column(SQLEnum(BudgedThresholdSourceEnum), nullable=False)
    ...
    __table_args__ = (
        Index("ix_budget_threshold_user_category_period", "user_id", "category_id", "period"),
    )
...
class FinancialPeriodSnapshot(Base):
    ...
    savings_rate: Mapped[Decimal] = mapped_column(Numeric(precision=5, scale=4), nullable=False) # savings / total_income
    
    # Aggregated data stored as JSONB
    category_spend: Mapped[list[dict[str, Any]]] = mapped_column(JSONB, default=list, nullable=False)
    budget_status: Mapped[list[dict[str, Any]]] = mapped_column(JSONB, default=list, nullable=False)
    ...
    __table_args__ = (
        Index("ix_financial_period_snapshot_user_period", "user_id", "period", "start_date"),
    )
